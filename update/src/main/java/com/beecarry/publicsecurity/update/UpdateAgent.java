...
package com.beecarry.publicsecurity.update;
...
public class UpdateAgent {
    ...
    public static class UpgradeJsonParser implements InfoParser {
        private int clientVersionCode = 1;

        public UpgradeJsonParser setClientVersionCode(int clientVersionCode) {
        ...
        public UpdateInfo parse(String source) throws Exception {
            ...
            JSONObject dataObject = object.has("data") ? object.getJSONObject("data") : object;
            JSONObject o = dataObject.has("data") ? dataObject.getJSONObject("data") : dataObject;

            if (o == null) {
                return info;
            }

            String versionCode = o.optString("version_code", "0");
            if (clientVersionCode < Integer.parseInt(versionCode)) {
                info.hasUpdate = true;
            } else {
                info.hasUpdate = false;
            }
            String update_type = o.optString("update_type");
            if (!TextUtils.isEmpty(update_type)) {
                if (update_type.equals("force")) {
                    info.isForce = true;
                } else if (update_type.equals("tip")) {
                    info.isIgnorable = true;
                } else if (update_type.equals("slience")) {
                    info.isSilent = true;
                }
            ...
            info.versionCode = o.optInt("version_code", 0);
            info.versionName = o.optString("version");
            info.updateContent = o.optString("description");

            info.url = o.optString("download_url");
            info.md5 = o.optString("filemd5");
            info.size = o.optLong("filesize", 0);
